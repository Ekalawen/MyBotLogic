...
BT_Noeud::ETAT_ELEMENT ScoreStrategie::execute() noexcept {
   ProfilerDebug profiler{ GameManager::getLogger(), string("ScoreStrategie::execute"), GameManager::getLoggerJSON(), GameManager::getMutex(), GameManager::getTempsDebutProgramme() };
   profiler << nom << endl;
...
bool ScoreStrategie::calculerScoresTilesPourNpc(Npc& _npc, const vector<int>& _tilesAVisiter) noexcept {
   ProfilerDebug profiler{ GameManager::getLogger(), "calculerScoresTilesPourNpc", GameManager::getLoggerJSON(), GameManager::getMutex(), GameManager::getTempsDebutProgramme(), false};
