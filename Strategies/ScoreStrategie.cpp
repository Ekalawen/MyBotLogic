...
BT_Noeud::ETAT_ELEMENT ScoreStrategie::execute() noexcept {
   ...
   for (auto& pair : manager.getNpcs()) {
      ...
      ss << "Durée calculerScoresEtCheminsTilesPourNpc = " << Minuteur::dureeMicroseconds(preCalcul, postCalcul) / 1000.f << "ms" << endl;
   ...
   ss << "Durée " << nom << " = " << Minuteur::dureeMicroseconds(pre, post) / 1000.f << "ms";
