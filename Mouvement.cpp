...
void Mouvement::apply(vector<Action*>& actionList, map<int, Npc>& npcs, Map& m) const noexcept {
    ...
    if (!isActivateDoorMouvement) {
        if (isNotStopped()) {
            ...
            npcs[getNpcId()].move(getDirection(), m);
        ...
        Porte& porte = m.getPorte(getTileSource(), getTileDestination());
